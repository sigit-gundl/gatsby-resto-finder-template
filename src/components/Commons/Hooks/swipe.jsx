...
// Swipe Utils
export function autoPlay(MyComponent) {
...
const checkIndexBounds = props => {
  ...
  warning(
    ...
    `swipe: the new index: ${index} is out of bounds: [0-${childrenCount}].`,
  );
};
...
class Swipe extends React.Component {
  ...
  render() {
    ...
    warning(
      ...
      `swipe: You are setting animateHeight to true but you are
        ...
        >
          {React.Children.map(children, (child, indexChild) => {
            ...
            warning(
              ...
              `swipe: one of the children provided is invalid: ${child}.
We are expecting a valid React Element`,
            ...
            return (
              <div
                ref={ref}
                style={slideStyle}
                className={swipeItemClass ? swipeItemClass : "swipe-item"}
                aria-hidden={hidden}
                data-swipeable="true"
              >
                {child}
...
Swipe.propTypes = {
   ...
   */
  swipeItemClass: PropTypes.string,
  /**
   * This is the inlined style that will be applied
   * on the slide component.
   */
